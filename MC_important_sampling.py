...
# target policy
PI = np.zeros(shape=(2, 11, 22))
# importance sampling total weight
C = np.zeros(shape=(2, 11, 22, 2))
# estimate values [usable ace][dealer's showing card (ace/10)][current sum][hit/stick]
Q = np.zeros(shape=(2, 11, 22, 2))

# exploration policy
B = np.zeros(shape=(2, 11, 22, 2))
# init exploration policy
for i in range(2):
    for j in range(11):
        for k in range(22):
            besta = np.argmax(Q[i][j][k])
            B[i][j][k][besta] = (EPSILON / 2) + (1 - EPSILON)
            B[i][j][k][1 - besta] = (EPSILON / 2)
...
for epi in range(NEPISODE):
    ...
    for state_action in reversed(this_episode):
        ...
        besta = np.argmax(Q[u][d][s])
        PI[u][d][s] = besta

        B[u][d][s][besta] = (EPSILON / 2) + (1 - EPSILON)
        B[u][d][s][1 - besta] = (EPSILON / 2)
